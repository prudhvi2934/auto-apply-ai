...
class SkillsAgentOutput(BaseModel):
    reordered_skills: list[str]
    suggested_grouping: dict[str, list[str]]
    skills_matched_to_jd: list[str]

class SkillGroup(BaseModel):
    category: str
    skills: list[str]

class SkillsGeminiSchema(BaseModel):
    """Flattened schema for Gemini - all fields at top level"""
    agent_name: str = "skills_agent"
    reordered_skills: list[str]
    suggested_grouping: list[SkillGroup] 
    skills_matched_to_jd: list[str]
    reasoning: str
    confidence: float
    ...
    async def generate(self, prompt: str, **override_config):
        ...
        def _call_genai():
            config=GenerateContentConfig(
                ...
                response_schema=SkillsGeminiSchema,
                temperature=0,
                **cfg  # for determinism
            )
            ...
            return SkillsGeminiSchema.model_validate_json(text)
...
class SkillsAgent:
    """Reorders and optimizes skills section"""
    
    def __init__(self, llm_client):
        self.llm = llm_client
    
    @track(name="skillAgent.create_skills", tags=["agent:SkillsAgent"],project_name="Auto-apply-AI")
    async def execute(self, task: AgentTask, resume: Resume, plan: TailoringPlan) -> AgentResult:
        """Execute skills optimization task"""
        
        prompt = f"""You are a SKILLS AGENT specializing in skills section optimization.
...
JD REQUIRED SKILLS/TOOLS (from planning):
Must-have: {', '.join(task.context.get('must_have', []))}
Nice-to-have: {', '.join(task.context.get('nice_to_have', []))}
Tools mentioned: {', '.join(task.context.get('tools', []))}

STRATEGY:
- Reorder skills to put JD-relevant ones first
- Group related skills logically
- Only include skills actually in the resume
- Suggest category groupings if helpful

Return JSON:
{{
    "reordered_skills": ["skill1", "skill2", ...],
    "suggested_grouping": {{"Category1": ["skill1"], "Category2": ["skill2"]}},
    "reasoning": "why this order",
    "skills_matched_to_jd": ["skill1", "skill2"],
    "confidence": 0.9
}}
"""
        ...
        grouping_dict = {
            ...
            for group in response.suggested_grouping
        }
        
        return AgentResult(
            agent_name=response.agent_name,
            output=SkillsAgentOutput(
            reordered_skills=response.reordered_skills,
            suggested_grouping=grouping_dict,
            skills_matched_to_jd=response.skills_matched_to_jd
            ),
...
async def main():
    ...
    skills_task = AgentTask.model_validate({
    ...
    skills_agent = SkillsAgent(llm_client)
    plan = await skills_agent.execute(skills_task,resume,tailoring_plan)
